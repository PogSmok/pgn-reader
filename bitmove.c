...
RawMove translateAlgebraic(char* c, bool movingSide) {
   if(*c == 'O') {
	...
	for(i = c+1; *i != '\0'; i++) {
		if(*i >= 'a' && *i <= 'h') {
			if(dest[0] != 0) org[0] = dest[0];
			dest[0] = *i;
		} else if(*i >= '0' && *i <= '9') {
			if(dest[1] != 0) org[1] = dest[1];
			dest[1] = *i;
		} else if(*i == 'x') {
			capture = true;
			dest[0] = *(++i);
			dest[1] = *(++i);
		} else if(*i == '=') promotedPiece = pieceToIndex(*(++i));
      else continue;	
	}
...
Board applyBitMove(BitMove bitMove, Board board) {
   ...
   //Castling have impossible piece indexes, 7 for short castle, 8 for long castle
