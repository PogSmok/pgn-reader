...
void readFile(char* path, char* pgnContent) {
   ...
   if(fptr != NULL) 
      while((c = fgetc(fptr)) != EOF && i < MAX_PGN_SIZE) pgnContent[i++] = c;
...
char* extractTags(char* path, char* pgnContent, char (*tagContent)[MAX_TAG_VALUE_LENGTH]) {
   ...
   tag = malloc(MAX_TAG_VALUE_LENGTH*sizeof(char));
...
void fillPositions(char (*rawMoves)[MAX_ALGEBRAIC_LENGTH], Board* positions, Board startingPosition) {
   ...
   for(int i = 0; rawMoves[i][0] != '\0'; i++) {
      ...
      positions[realIndex] = applyBitMove(rawToBit(translateAlgebraic(rawMoves[i], previousPosition.turn)), previousPosition), previousPosition = positions[realIndex];
